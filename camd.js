...
function autowrap(info, conf) {
  ...
  var prefix = 'C5.define(' + (file.skipDepsAnalysis ? '\'' + (file.moduleId || file.id)+ '\', [], ' : '') + 'function(require, exports, module) {\n\n';
  var affix = '\n\n});';
    ...
    if (!forgetDefine && module) {
      ...
      if (count) {
        ...
        if (module.idLoc) {
          ...
          start = converter(start.line, start.column);
          start += /^C5\.define\s*\(/.exec(content.substring(start))[0].length;
...
function traverse(ast, scopes, gs, enter, leave) {
  estraverse.traverse(ast, {
    enter: function(current, parent) {
      ...
      // 检测 C5.define(id?, deps?, factory);
      if (current.type === 'CallExpression' &&
        current.callee.object &&
        current.callee.object.type === 'Identifier' &&
        current.callee.object.name === 'C5' &&
        current.callee.property &&
        current.callee.property.type === 'Identifier' &&
        current.callee.property.name === 'define' &&
...
function findRef(scope, ident) {
  ...
  while ((ref = refs[i++])) {
    ...
    if (ref.identifier === ident) {
      return ref;
    }else if(ref.identifier === ident.object) {
      return ref; 
    }
  }
